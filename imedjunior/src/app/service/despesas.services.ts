...
export class DespesaService {
    private url: string = `http://localhost:8000/despesa/`;
    constructor(private http: Http) {
    }
    errorHandler(error: any): void {
        console.log(error)
    }
    save(despesa: any): Observable<any> {
        var cabecalho = new Headers();
        cabecalho.append('Authorization','JWT '+sessionStorage.getItem("token"));
        cabecalho.append('Content-Type', 'application/json');
        return this.http
            .post(this.url, JSON.stringify(despesa), { headers: cabecalho })
            .map(response => response.json())
            .catch((error: any) => Observable.throw(this.errorHandler(error)))
    }
    update(id: number, despesa: any): Observable<any> {
        var cabecalho = new Headers();
        cabecalho.append('Authorization','JWT '+sessionStorage.getItem("token"));
        cabecalho.append('Content-Type', 'application/json');
        return this.http
            .put(this.url + id + "/",JSON.stringify(despesa), { headers: cabecalho })
            .map(response => response.json())
            .catch((error: any) => Observable.throw(this.errorHandler(error)))
    }
